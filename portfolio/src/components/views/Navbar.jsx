...
const Navbar=()=>{
    ...
    return (
        <nav className="h-20  w-full bg-blue-600 font-semibold fixed top-0 left-0 p-4 md:px-4 lg:px-36 text-white tracking-wide shadow-md shadow-bottom">
            <FontAwesomeIcon icon={faBars} onClick={toggleMenu} className='m-4 float-right md:hidden w-7 h-7'/>
            <div className="md:flex justify-between">
                <span className="block text-2xl mt-2 mr-40 md:flex md:mt-1 md:text-2xl md:ml-5 md:mr-20 lg:text-3xl lg:mr-0 mt-1 lg:ml-10">DenisRoldan.dev</span>
                <ul className={`${isMenuOpen ? 'block' : 'hidden'} md:flex my-2 mr-8 mt-3 text-lg hover:transition-all duration-300}`}>
